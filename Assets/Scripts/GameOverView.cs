...
public class GameOverView : MonoBehaviour//clase que se relaciona al canvas del gameOver, para controlar las puntuaciones del personaje
{
    ...
    void Update()
    {
        ...
        {//si estoy en el juego, que se vayan actualizando las puntuaciones
            int coins = GameManager.sharedInstance.collectedObject;//obtenemos el contador que se actualiza en el GameManager
            float score = controller.GetTravelledDistance() + (coins * 3);//en la funcion del controlador del personaje llamada getTravelledDistance, podemos obtener la distancia del personaje que recorrio en x
